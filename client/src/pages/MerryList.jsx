...
import { ButtonGoToChat } from "@/components/base/button/ButtonGoToChat";
import { ButtonProfile } from "@/components/base/button/ButtonProfile";
import {
  TypographyH1,
  TypographySmall,
...
function MerryCard(props) {
  ...
  const merryMatchButton = (status) => {
    if(status.toLowerCase() === "merry"){
      ...
      return  <div className="border-2 border-pred-500 rounded-full flex items-center justify-center py-1 px-4 w-fit">
      <svg
        width="21"
        height="12"
        viewBox="0 0 21 12"
      ...
      return <div className="border-2 border-pgray-500 rounded-full flex items-center justify-center py-1 px-4 w-fit">
  ...
  return (
    <>
      ...
      <div className="flex border-b h-[180px] items-center py-28 my-3">
        ...
        {merryMatchButton(profile.status)}
          {/* Merry match! เดี๋ยวมาทำ logic */}
          ...
          <div className="w-10/12 justify-between flex flex-row-reverse mt-5">
            <button className="w-10 h-10 bg-pred-500 rounded-2xl shadow-3xl">
              <svg
                width="44"
                height="42"
                viewBox="0 0 44 42"
